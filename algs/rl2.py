...
def calulate_best_action(x, y, actions, rewards, transition, v_last):
    max_action = None
    max_val = -inf
...
def update_policy(v, goals, world, actions, transition, rewards):
    ...
    for x, y in get_states(world):
        ...
        if (x, y) in goals:
            ...
            policy[(x, y)] = "G"
            continue
        max_action, max_val = calulate_best_action(x, y, actions, rewards, transition, v_last)
...
def value_iteration(world, costs, goals, actions, gamma, transition=1.0, e=0.01, max_iters=100, debug=False):
    ...
    while t < max_iters:
        ...
        if debug:
            print(f"T={t}")
            pretty_print_policy(cols, rows, policy, world)
        if delta < e:
            return policy
        t += 1
    return None

def pretty_print_vi(cols, rows, v, v_last, rewards):
    return 
